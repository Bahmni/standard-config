...
FROM (
			...
			FROM (
					SELECT distinct Id,Patient_Identifier, ART_Number, Patient_Name, Age ,age_group, Gender, TPT_Start_Date, TPT_Completion_Date,Completed_TPT, sort_order
				FROM
					(
						(SELECT distinct Id, patientIdentifier AS "Patient_Identifier", ART_Number, patientName AS "Patient_Name", Age ,age_group, Gender, "Yes" as Completed_TPT, sort_order
                FROM
