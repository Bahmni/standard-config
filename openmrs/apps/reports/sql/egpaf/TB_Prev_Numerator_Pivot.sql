...
FROM (
			...
			FROM (
					SELECT distinct Id, Patient_Identifier, ART_Number, Patient_Name, Age , Gender, TPT_Start_Date,age_group, sort_order, TPT_Completion_Date, 'Yes' as Completed_TPT
						FROM
							(
								(SELECT distinct Id, patientIdentifier AS "Patient_Identifier", ART_Number, age_group, sort_order, patientName AS "Patient_Name", Age , Gender
										FROM
											...
											from obs o
													...
													AND o.person_id in 
														(
															-- Completed TPT in the previous Period
